  ...
  try {
    ...
    if (cUser) {
      ...
      if (existingUser) {
        ...
        try {
          await accountLogin(state, commands, prefix, botName, [admin]);
          res.status(200).json({
            success: true,
...
async function accountLogin(state, enableCommands = [], prefix, botName, admin = []) {
  return new Promise((resolve, reject) => {
    login({
      ...
      addThisUser(userid, enableCommands, state, prefix, botName, admin);
      try {
...
async function addThisUser(userid, enableCommands, state, prefix, botName, admin, blacklist) {
  const configFile = './data/history.json';
  ...
  try {
    for (const file of fs.readdirSync(sessionFolder)) {
      ...
      try {
        ...
        if (enableCommands) await accountLogin(state, enableCommands, prefix, botName, admin, blacklist);
      } catch (error) {
